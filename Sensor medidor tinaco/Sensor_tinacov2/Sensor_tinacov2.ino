//Sensor para nivel de agua-----------------------Daniel Torres--- 19/04/21

//Asignacion de pines
//Lectura de sensores cada 10s
...
#define botonpresionado 6  //Tiempo de presionado (/2) necesario para activar bomba
...
void loop() {
  ...
  if ((unsigned long)(currentMillis - previousMillis) >= interval) //ciclo principal
  {
    ...
    if(bombaactivada==true && (condicion>=4 && condicion<6)){
      cuentabomba++;
    }
    else{
      cuentabomba=0;
...
void bomba(){      //Chequeo de sensores y bomba activa
  ...
  if(condicion==4 || condicion==5 || paroemerg==true){ //LLeno
     if(cuentabomba>=tiempoparo || condicion==5 || paroemerg==true){
      digitalWrite(rele, LOW);
      bombaactivada=false;
      digitalWrite(led3, LOW);
      unicociclo=true;
